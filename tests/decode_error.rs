...
//! Tests for `deserialize_with_path` — response decoding with `serde_path_to_error`
//! path tracking and `HonchoError::Decode` source-chain wiring.
...
/// Assert the result is `Err(HonchoError::Decode { path, source })` and return
/// references to both. Verifies that the `#[source]` attribute is reachable via
/// the `std::error::Error::source` chain (not only via field destructuring).
...
fn decode_valid_json_returns_ok() {
    let input = br#"{"id":"x"}"#;
    let result: RequiresString = deserialize_with_path(input).expect("valid JSON should decode");
...
fn decode_malformed_json_within_field_reports_field_path() {
    // Genuinely malformed JSON (invalid literal `tru}` after the colon): even
    // syntax errors are path-tracked to the field being parsed.
    let input = br#"{"id": tru}"#;
    let result = deserialize_with_path::<RequiresString>(input);
    let (path, source) = expect_decode_err(&result);
    assert_eq!(path, "id");
